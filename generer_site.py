...
<style>
  /* Le journal sous la charte commune (chantier 3, 12/08/2026).
     ...
     pointent maintenant vers ceux du kit. --bg, --line et --orange
     portent deja le meme nom, la charte les sert donc directement.
     Les quatre couleurs de phase ne sont PAS touchees : ce sont des
     accents de contenu, pas de la charte. */
  :root{--surface:var(--bg-2);--surface2:var(--bg-3);--text:var(--fg);
        --muted:var(--fg-2);--faint:var(--fg-3);}

  /* Seuls les JETONS de la charte sont charges, pas sa mise en page :
     ce site a la sienne. Essaye le 12/08/2026 avec verdier.css entier,
     la page grandissait de 258 px — `.hero h1` du kit est plus specifique
     que le `.hero-titre` d'ici, qui perdait. Dix-sept selecteurs se
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  /* :not(.topbar) — cette regle visait TOUS les <header>. Mesure au
     navigateur le 18/08/2026 : la barre de l'atelier heritait de ses
     70+44 px et faisait 175 px de haut au lieu de 60, recouvrant les
     onglets qu'elle devait laisser passer dessous. Le heros garde son
     air, la barre reprend sa hauteur. */
  header:not(.topbar){padding:70px 0 44px;border-bottom:1px solid var(--line);}
  ...
  /* Le fond suit le theme. Il valait rgba(19,17,14,.95) -- l'ancienne
     palette sombre du journal, ECRITE EN DUR : la diffusion de la charte
     remplace des NOMS de jetons, elle ne peut rien contre une valeur
     litterale. D'ou une bande noire en travers d'une page devenue claire.
     Meme forme que la barre du portail, dans kit/verdier.css. */
  /* Les onglets collent SOUS la barre de l'atelier, et non a top:0 ou
     ils passaient derriere elle (leur z-index vaut 10, le sien 50) et
     disparaissaient des qu'on defilait.

     La valeur est MESUREE par majMargeAncre, pas ecrite en dur : depuis
     le 18/08/2026 la barre du kit a une hauteur MINIMALE de 60 px et non
     fixe — elle grandit si ses liens passent a deux lignes. Un 60 en dur
     redonnerait le chevauchement le jour ou elle grandit. */
  .nav{position:sticky;top:var(--haut-barre,61px);background:color-mix(in srgb,var(--bg) 88%,transparent);backdrop-filter:saturate(140%) blur(10px);border-bottom:1px solid var(--line);z-index:10;}
...
#accueil.show, #tl.show, #recit.show, #maj.show, #doc.show, #gloss.show { display: block; }
  /* z-index:51, au-dessus de la barre (50) : a 20 ce trait de 3 px
     passait derriere son fond translucide et son flou. */
  .recit-progress{position:fixed;top:0;left:0;height:3px;background:var(--orange);width:0%;z-index:51;transition:width .1s linear;pointer-events:none;}
  ...
  .doc-liste li code{background:var(--bg-3);border:1px solid var(--line);border-radius:4px;padding:1px 6px;font-family:monospace;font-size:13px;color:var(--orange);}
  ...
  @media print{
    .nav,.filters,.topbar,.tab-actions,.recit-progress,.back-to-top,.search-box,#tl,#recit,#maj,#accueil,footer{display:none!important;}
  ...
  /* MARGE DE DEFILEMENT DES ANCRES — valeur posee par le JS (voir
     majMargeAncre), jamais devinee : la hauteur des barres collantes
     change avec l'onglet et avec la largeur de la fenetre.

     Mesure au navigateur le 18/08/2026, sur la version d'AVANT la barre
     de l'atelier : une ancre #v-... amenait deja l'etape ENTIEREMENT
     derriere la barre d'onglets — etape haute de 103 px, barres de
     192 px, item pose a 0. Le defaut ne vient donc pas de la barre
     commune ; elle l'aggravait de 60 px, d'ou la reparation ici.

     Le repli de 264 px sert si le JS ne tourne pas : c'est la pile
     mesuree sur l'onglet « Timeline » en 1280 px de large, plus un peu
     d'air. */
  .item,.doc-section,.month{scroll-margin-top:var(--marge-ancre,264px);}
...
<body>
<!-- LA BARRE COMMUNE DE L'ATELIER — gabarit kit/entete.html du depot
     « site », son CSS vient de kit/verdier-entete.css.

     Ce qu'elle repare, releve le 18/08/2026 : ce journal citait
     `atelierduverdier.fr` cinq fois, et les CINQ etaient
     `laser.atelierduverdier.fr`. Aucun lien vers la racine du domaine.
     Depuis la bascule du 12/08, la racine sert le portail — un visiteur
     arrive d'un reel Instagram n'avait donc aucune porte de sortie vers
     l'atelier. C'etait ca, le decousu ; pas le sous-domaine.

     Le bouton de theme est ICI et plus en rond flottant : a top:14px il
     tombait dans la bande de la barre, et derriere elle. -->
<header class="topbar">
  <div class="wrap">
    <a class="brand" href="https://atelierduverdier.fr/" aria-label="Atelier du Verdier">
      <img class="chapeau" src="kit/verdier-chapeau.svg" alt="">
      Atelier du Verdier
      <span class="brand-sub">Journal PrintNC</span>
    </a>
    <button class="nav-btn" type="button" aria-label="Ouvrir le menu"
            aria-controls="menu-principal">☰</button>
    <nav class="navlinks" id="menu-principal">
      <a href="https://atelierduverdier.fr/">Accueil</a>
      <a href="https://atelierduverdier.fr/#logiciels">Logiciels</a>
      <a href="https://atelierduverdier.fr/#atelier">L'atelier</a>
      <a href="https://atelierduverdier.fr/projets/">Projets</a>
      <a href="https://github.com/atelierduverdier" target="_blank" rel="noopener">GitHub</a>
    </nav>
    <button class="theme-btn" id="theme-toggle" title="Mode jour/nuit"
            aria-label="Basculer entre le theme clair et le theme sombre">☀</button>
  </div>
</header>
<header>
  <div class="wrap">
...
<script src="kit/verdier.js"></script>
<script>
...
// l'atelier est fixe (60 px) mais celle des onglets grandit avec la
// rangee des mois, la recherche et les filtres, et grandit encore quand
// les onglets passent a la ligne en etroit. On la MESURE.
function majMargeAncre() {
    var barre = document.querySelector('header.topbar');
    var onglets = document.querySelector('nav.nav');
    if (!onglets) return;
    var hb = barre ? barre.offsetHeight : 61;
    document.documentElement.style.setProperty('--haut-barre', hb + 'px');
    document.documentElement.style.setProperty('--marge-ancre',
        (hb + onglets.offsetHeight + 12) + 'px');
}
majMargeAncre();
window.addEventListener('resize', majMargeAncre);
...
// Theme : la LOGIQUE vient de kit/verdier.js (data-theme + cookie de
// domaine, donc le reglage suit le visiteur d'un sous-domaine a l'autre).
// Ici on ne garde que l'icone, propre a ce site.
const toggle = document.getElementById('theme-toggle');
function majIcone() {
    var sombre = document.documentElement.getAttribute('data-theme') === 'dark'
              || (!document.documentElement.getAttribute('data-theme')
                  && matchMedia('(prefers-color-scheme: dark)').matches);
    toggle.innerText = sombre ? '☀' : '🌙';
}
majIcone();
new MutationObserver(majIcone).observe(document.documentElement,
