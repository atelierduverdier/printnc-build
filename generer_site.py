  ...
  <div class="wrap">
    <div class="adv-rate" id="advRate">
      ...
      <button class="adv-rate__btn" id="btn-useful">
        <svg viewBox="0 0 24 24" width="18" height="18"><path d="M12 4.5C7 4.5 2.73 7.61 1 12l2.89 2.89c.76-.77 1.81-1.25 2.97-1.25 2.34 0 4.24 1.9 4.24 0 1.17-.48 2.22 2.97L12 23.5l2.15-2.15c-.77-.75-1.25-1.8-1.25-2.97 0-2.34 1.9-4.24 4.24-4.24 0-2.34-1.25-2.97-1.25-2.97z"/></svg>
        Utile
      </button>
...
if (btnUseful) {
    if (localStorage.getItem('site_useful_voted')) btnUseful.classList.add('is-voted');
...
// --- Deep linking au chargement ---
window.addEventListener('load', () => {
    if (window.location.hash) {
        const target = window.location.hash.substring(1);
        if (document.querySelector(`[data-month="${target}"]`)) {
            switchTab(target);
        }
    }
});
